...
export class BitgetAdapter extends ExchangeAdapter {
  ...
  private createClient(): Exchange {
    ...
    const BitgetCtor = ccxt.bitget
    return new BitgetCtor(opts)
  }
  ...
  private async resolveCcxtSymbol(client: Exchange, symbol: SymbolPair): Promise<string> {
    ...
    if (markets.length === 0)
      throw new Error(`Bitget 找不到合约市场: ${symbol}`)
  ...
  async fetchFundingRate(symbol: SymbolPair): Promise<FundingRate> {
    ...
    throw new Error('Bitget 不支持通过 ccxt 获取资金费率')
  }
  ...
  async fetchTickerPrices(symbol: SymbolPair): Promise<TickerPrices> {
    ...
    const index = t.indexPrice ?? t.info?.indexPrice ?? t.info?.index ?? mark
    return {
